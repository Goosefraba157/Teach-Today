...
- Section 4 Student Stage now embeds the authentic Reader PDF page in place, fitted to show the complete portrait page on a widescreen display. It derives the public PDF only from the sanitized Reader/page fields, keeps the generated clean word page behind it until the PDF loads, and caches all twelve Reader PDFs for offline fallback after installation. The teacher charting surface, records, Follow Lesson routing, and native sanitizer are unchanged. Commit `b201fa2` was published in GitHub Pages deployment 86 and the public site was verified serving `section4-reader-pdf-1` on 2026-08-23.
