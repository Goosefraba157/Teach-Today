...
- A `Laser` control now sits directly below Notes in the presentation dock. The bright red point follows the mouse without requiring a click, leaves a short fading movement tail, and allows normal clicks on lesson buttons because its canvas is visual-only. Touch taps still activate lesson controls, while a moved touch is reserved for laser scooping so it does not scroll the page. It never enters lesson records or permanent annotation state. Notes, laser, and active global ink are mutually exclusive so their canvases cannot compete for input.
