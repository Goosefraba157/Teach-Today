...
- Home now includes a compact official lesson-history control for the selected current-year group. A teacher can select a prior lesson, edit its lesson number and Day 1/Day 2 dates, and save only after confirmation; duplicate lesson numbers receive an additional warning. A group-level reset preserves Teach Today plans as excluded test records, clears official continuity to zero, and leaves student profiles, charting, assessment records, and archived years unchanged.
...
- Exact next step: wait for GitHub Pages/service-worker refresh, select Group 1 - Jade, use Lesson history → Reset this school year, confirm the open lesson disappears and the next saved lesson is Lesson 1, then create/use Demo Group for testing. Smoke-test prior-lesson date/number correction and presentation navigation afterward.
