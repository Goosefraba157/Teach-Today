...
- The external scene never falls back to a teacher mirror. Loading, network, and web-process failures show a neutral Classroom Stage screen and retry without exposing the teacher surface. Firebase remains entirely owned by hosted V1; the native shell adds no Firebase configuration, records, or migration.
...
- The new webview shell has not yet been installed on the physical iPad. Command-line signing in the current macOS login has no available Apple Development identity, so the already-installed A/B proof was left untouched. Firebase popup sign-in and the new shell's AirPlay behavior therefore remain hardware checks, not completed claims.
...
- Exact next step: open `ios/TeachTodayStageProof/TeachTodayStageProof.xcodeproj` in the macOS login that has the Personal Team available, select that team locally, and run the updated `Teach Today Stage` target on the connected iPad. First verify Firebase popup sign-in and private revision loading without changing browser data; then connect AirPlay, reopen the app, enable Follow Lesson, and check Sections 1-9 plus disconnect/reconnect and background/foreground recovery. Keep the browser Present/Stage route as the classroom recovery path until those checks pass.
