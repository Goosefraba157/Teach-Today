...
- Follow Lesson projects every guided or discovery scene as a separate student-safe browser/native Stage view while teacher cues remain private. Browser verification audited all 24 discovery scenes, prompt/reveal behavior, all sound-card and mouth assets, notebook layout, suffix proportions, original-intro preservation, and live Stage synchronization with no lesson overflow. JavaScript syntax and sync-safety tests pass, and the native Stage shell builds successfully for a generic iOS simulator with signing disabled. Exact next step: classroom-test both buttons and decide whether the discovery wording/pacing should replace the guided default before building a shorter Section 7 auditory-discovery intro.
