...
- A `Laser` control now sits directly below Notes in the presentation dock. The bright red point follows mouse or stylus movement without requiring a click, leaves a short fading movement tail, and allows normal clicks on lesson buttons because its canvas is visual-only. With Laser left on, a finger scrolls the lesson normally while the stylus controls the laser and can scoop without intentionally scrolling. It never enters lesson records or permanent annotation state. Notes, laser, and active global ink are mutually exclusive so their canvases cannot compete for input.
