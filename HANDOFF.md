...
- Home planner audit: the recommendation and per-student evidence are useful and data-driven, but the current page mixes group defaults, today’s draft, and reusable/historical actions. The duplicated lesson-format picker should be consolidated; Build Lesson overlaps Start Teaching; Save as Template currently stores templates without any retrieval UI; Duplicate Previous copies the prior lesson’s exact selected content rather than generating fresh words from the same page.
...
- Exact next step: after GitHub Pages publishes the startup-loop fix, refresh both hosted browsers once and confirm the repeated flicker stops. Then use Demo Group to verify one real edit produces at most one update on the idle device. After that, simplify Home around one Current Lesson workspace: recommendation summary, Customize, and Start Teaching, with advanced/reusable actions moved out of the primary path.
