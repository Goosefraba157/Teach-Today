...
- Exact next step: publish the Home/realtime-sync update, refresh one hosted signed-in tab, and confirm Home shows “Sign out” plus the account name. Then open a second signed-in browser, make a harmless Demo Group change on only one device, and confirm the idle device reloads the completed revision. Finally create unsynced Demo Group edits on both devices and confirm neither silently overwrites the other.
