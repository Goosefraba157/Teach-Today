...
- Exact next step: after GitHub Pages publishes, refresh the hosted app and verify the compact Recommended Plan, Customize format selector, and Redo Charting Page behavior with Demo Group. Confirm a reteach keeps Section 4’s page, changes Section 2/2B practice words, uses prior concepts for review, and receives a new number only when Start Teaching saves it. Also confirm the Firebase flicker remains stopped across two browsers.
