...
- Exact next step: wait for GitHub Pages/service-worker refresh, close extra signed-in Teach Today tabs, refresh one hosted tab, and run Firebase + local sync now. Confirm the Firebase status reports success (or a one-time legacy repair followed by success), then reopen a second browser and verify stale-tab conflicts are blocked. Continue date/default and Demo Group smoke tests afterward.
