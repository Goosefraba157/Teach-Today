...
assert.match(source, /function pmColumns\(type\)/);
assert.match(source, /Lesson \$\{column\.lesson\}/);
assert.doesNotMatch(source, /pmInPeriod/);
assert.doesNotMatch(source, /pmWeeks/);
assert.match(source, /data-copy-percent/);
assert.match(source, /navigator\.clipboard\.writeText\(value\)/);
assert.match(source, /record\.categories\?\.\[category\]/);
assert.match(source, /pmNormalize\(name\) === "5realwords"/);
