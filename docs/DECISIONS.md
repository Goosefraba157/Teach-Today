...
- **Independent automatic backups:** After an authoritative Firebase revision is confirmed, Teach Today may copy that exact shared payload to `Teach Today Backups/Daily` and `Weekly` in Google Drive and to `On My iPad/Teach Today Stage/Backups` through the native shell. These destinations are recovery archives, never synchronization authorities, and they must not auto-restore or write back to Firebase.
