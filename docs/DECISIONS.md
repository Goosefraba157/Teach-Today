...
- **Stage following:** `Follow Lesson` may switch Stage from the teacher's visible section, but only sections with an approved student renderer display lesson content. Unsupported sections fail closed to the neutral privacy screen, and any manual Stage mode remains an explicit override.
