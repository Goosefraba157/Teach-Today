...
- Added safe editing for an unfinished saved lesson. The Home continuity card now opens the existing lesson in the normal planner with its saved selections; saving writes back to the same plan and lesson IDs, keeps operational progress/student evidence intact, records a revision, and opens the updated lesson in Present mode. The workflow does not close the lesson or create another history entry.
- Reduced accidental-click risk in the iPad Stage Records panel. Only the normal **Back up today** and **Connect Google Drive backup** controls remain active there; maintenance, recovery, Firebase, import/export, audio, and local-folder controls stay visible but disabled and gray. The browser version retains the controls for deliberate future maintenance, and no records or backup behavior changed.
- Fixed the physical Stage shell's blocked Google authorization window. The existing full-screen native popup controller remains the only new-window presentation path; the main WKWebView now explicitly allows JavaScript-created windows so Firebase Auth can open Google's consent flow after a teacher tap. The latest native Daily checkpoint was preserved off-device before the in-place rebuild; no application or student data was changed.
- Clarified the Stage Drive/Firebase boundary in the interface. The Drive backup control now uses a recognizable blue/green/yellow treatment, its status points to the correct authorization control, and Stage visibly reports Firebase as paused rather than presenting a historical sync time as active. Backup JSON continues to upload directly to the Google Drive API; Firebase Auth only brokers Google's temporary consent token, while Stage's Firestore read/write/listener/reconciliation paths remain disabled. Refreshed the marker to `20260901-drive-boundary-1`.
- Added the second, off-device backup layer requested for accidental app deletion. After explicit Google authorization, Stage writes the same complete checkpoint to verified dated Daily/Weekly files in both iPad Files and the private `Teach Today Backups` Drive folder. Local backup never depends on Drive; Drive upload runs automatically only while its temporary authorization is active, and Records reports separate verified iPad and Drive times. Refreshed the marker to `20260901-drive-backup-1`; Firebase remains paused in Stage.
- Made **Back up today** the single visually primary Records backup control using a filled green treatment and raised shadow. Other manual download, recovery, transfer, and Firebase buttons remain visually secondary; no behavior or data changed. Refreshed the hosted/offline marker to `20260901-daily-backup-primary-1`.
- Guaranteed an automatic verified iPad Files backup on the first Stage launch/save of every calendar day, including days when the underlying state is unchanged. Automatic Stage checkpoints are now local-only and never check optional Drive permission; **Back up today** remains the explicit force-now control. Refreshed the hosted/offline marker to `20260901-daily-backup-1` without changing student data or the backup schema.
- Made Stage's **Back up today** control force a complete iPad Files snapshot without Firebase or Google Drive. Concurrent backup requests now wait rather than silently returning, the native shell continues to atomically write and reread both dated Daily and Weekly JSON files, and Records shows the last verified native-backup time plus the exact Daily filename instead of conflating it with the separate manual download.
- Confirmed the complete backup artifact contains the current application state and planner overrides, including rosters/student evidence, charting, Sections 6–8 observations, dictation, attendance, current and saved lessons, and lesson history. Refreshed the hosted/offline marker to `20260901-backup-verification-1`; no student data, Firebase data, or existing backup was modified by the code change.
- Identified the physical-iPad Section 4 Stop/student-switch failure as WebKit storage exhaustion rather than Firebase or timer behavior. The real checkpoint held 2.60 MB of UTF-8 state—approximately 5.20 MB as a WebKit string—of which 942 KB was never-read duplicate revision lesson snapshots and 322 KB was redundant full-JSON fingerprints.
- Compacted saved-version history to the ID/time/reason metadata the UI actually reads and replaced full lesson fingerprints with a compact deterministic hash. The real checkpoint projects to 1.35 MB after compaction, while current lessons, charting, Sections 6–8 evidence, attendance, profiles, and the protected full JSON remain intact. Refreshed the marker to `20260901-stage-stability-2`; all 31 tests and syntax/diff checks pass.
...
- Published the Stage interaction repair at core commit `e9c5d8d`, verified the public page and offline-cache markers, installed the same signed bundle over the connected iPad app, and launched it. Post-install device inspection confirmed all three dated native daily backups remain in the app container, including the current 2.5 MB checkpoint.
- Removed the redundant post-open lesson save from Stage Continue. The stable-ID opener already persists the selected plan; avoiding the second lookup prevents a freshly loaded iPad lesson from racing itself and falsely reporting a reopen failure. Presentation-mode startup is now non-blocking, with the loaded lesson retained and an ordinary-view fallback if that UI transition fails. Refreshed the hosted/offline cache marker and passed all 23 safety tests plus JavaScript syntax.
- Published the non-blocking Stage resume repair to GitHub `main` at core commit `8583a30`; Pages deployment 149 completed successfully and the public `20260901-stage-continuity-3` HTML and offline-cache markers were verified.
...
- Published the continuity repair to GitHub `main` at core commit `132cb01` and verified that GitHub Pages serves the new `20260901-stage-continuity-2` asset marker.
...
- Published the one-tap Student Profile picker at core commit `3c0ac2a`; GitHub Pages deployment 146 completed successfully and the public profile plus offline-cache version markers were verified.
...
- Browser-verified the complete Home flow with isolated synthetic data: close-as-incomplete exposed Customize, `Start Planned Lesson` opened the exact new lesson, and a fresh Home load followed by `Continue Lesson` reopened that saved lesson.
- Published the continuity repair at core commit `b213567`; GitHub Pages deployment 138 completed successfully and the public HTML plus offline-cache version markers were verified.
- Made Home close-as-incomplete safe to repeat when background synchronization closes the lesson after Home rendered but before the teacher confirms. An already terminal lesson now clears only its stale active pointer and reveals Customize instead of leaving the red “already closed or could not be found” dead end; the terminal status, timestamps, evidence, and history remain unchanged.
- Published the idempotent-close follow-up at core commit `9476756`; GitHub Pages deployment 140 completed successfully and the public `continuity-actions-2` page and offline-cache markers were verified.
- Fixed iPad back/forward-cache restoration after visiting Profile or another app page. A restored Home page now reloads current saved state and discards cached close confirmations and red failure messages instead of resurrecting an obsolete screen; lesson and student records are not rewritten.
- Browser-verified the exact reported sequence with isolated Demo data: open Home close confirmation, enter Profile, press Back, and confirm Home returns with the cached message gone while the actual current lesson remains intact.
- Published the Profile-to-Home restoration fix at core commit `a569ae2`; GitHub Pages deployment 142 completed successfully and the public `continuity-actions-3` page and offline-cache markers were verified.
- Removed the native-only Firebase upload/reconciliation pause that left the Stage shell on a separate persistent local lesson state while Safari used the current cloud state. Stage now follows the same automatic backup-first revision, stable-ID merge, and cloud-listener path as the browser; native projection and private Files backup behavior remain unchanged.
- Added native-sync regression coverage plus a concrete cross-surface merge case proving a browser-closed lesson and cleared active pointer combine with a distinct Stage observation without losing either valid change.
- Published the Stage sync-parity fix at core commit `bce8d3f`; GitHub Pages deployment 144 completed successfully and the public HTML and offline-cache version markers were verified.
- Replaced the accumulated Student Profile interface with a clean, read-only profile organized into Overview, Section 4 Charting, Sections 6–8, Timeline, and Legacy archive views. The active school year is the default, older years are explicitly read-only, and only groups assigned to the selected year can populate its student selector.
- Removed Student Profile's independent whole-store writes and added a same-device state notification so an open profile refreshes after Home or Lesson/Present saves. Home and Lesson/Present keep their existing save path; no student, lesson, Firebase, backup, or storage record was migrated, deleted, or uploaded by this release.
- Added a private, device-generated legacy JSON export before the later transactional-storage migration. It preserves the current local roster, yearly groups, lessons, charting, Sections 6–8 evidence, attendance, and membership fields without adding private data to the repository.
- Combined duplicate Section 8 evidence in the new read-only display, kept Section 6 free of HFW trouble-spot summaries, hid demo/sample groups from the profile selector, refreshed the offline cache, and added regression coverage for the new profile boundary and cross-page notification.
- Published the Student Profile restart to GitHub `main` at core commit `594a344`; Pages deployment 136 completed successfully and the live profile version marker was verified.
- Added a generated, curriculum-only word index from the official Wilson Dictation Book word pages: 34 substeps, 87 pages, 4,109 unique real words, and 424 marked nonsense words. The generator validates extracted words against the reviewed Reader chart index and stores no local source paths or student data.
...
- Published the Section 6–8 instructional summary to GitHub `main` at core commit `3a2ade5`; Pages deployment 132 completed and the live Student Profile and offline-cache version markers were verified.
...
- Published the Home data shortcuts and immediate Section 6–8 observation release to GitHub `main` at core commit `e4a9e13`; Pages deployment 130 completed and the live Teach Today, Student Profile, and offline-cache markers were verified.
...
- Published the focused Section 3 planner, Fat Stack, Lesson Deck, and HFW controls to GitHub `main` at core commit `05a1b7e`; Pages deployment 128 completed and the live `20260830-section3-fat-stack-1` CSS/JavaScript markers and clean console were verified.
...
- Tightened Reader concept grouping to the selected level or its established fallback, exposed all 17 indexed N-page substeps through the curriculum helper, refreshed offline asset versions, and verified 1.4, 2.1, Section 2B, a no-N 3.5 case, and selection preservation in the local browser.
- Published the N-page filters and reviewed label corrections to GitHub `main` at commit `248c6cb`; Pages deployment 126 completed successfully and the live `20260830-review-n-pages-1` HTML and JavaScript markers were verified.
