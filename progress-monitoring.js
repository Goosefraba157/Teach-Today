...
function pmDate(value) { const date = value ? new Date(value) : null; return date && !Number.isNaN(date.getTime()) ? date : null; }
...
function pmInPeriod(record) { const value = pmRecordDate(record); const day = value ? String(value).slice(0, 10) : ""; return day >= pmPeriod().start && day <= pmPeriod().end; }
function pmWeeks() {
...
function pmRealRecords(student) { const results = []; pmGroups().forEach((group) => (group.dictationAssessments || []).forEach((record) => {
  ...
  const words = record.categories?.["5 real words"] || [];
  if (!words.length) return;
  const allowed = new Set(words.map(pmNormalize));
  const direct = record.missesByCategory?.["5 real words"];
  const source = Array.isArray(direct) ? direct : (group.encodingObservations || []).filter((mark) => mark.section === "section8" && (mark.planId === record.planId || mark.lessonId === record.lessonId) && mark.category === "5 real words" && allowed.has(pmNormalize(mark.item))).map((mark) => mark.item);
